...
const Menu = () => {
  ...
  const addToCart = (itemKey) => {
    if (quantities[itemKey] === 0) {
      alert('Please enter a quantity before adding the item to the cart.');
      return;
    }
    ...
    setQuantities(prevQuantities => ({ ...prevQuantities, [itemKey]: 0 })); 
  };

  const handleViewOrder = () => {
    if (cart.length === 0) {
      alert('Please select at least one item to view your order.');
      return;
    }
    navigate('/Vieworder', { state: { cart } });
  };
